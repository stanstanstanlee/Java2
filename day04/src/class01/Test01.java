	...
	public static void main(String[] args) {
		...
		while(true) { // 몇번 반복할지 모르기 때문
			...
			while(true) {
				...
				num=sc.nextInt();
				// num (지역)변수는 60번의 while블록에서 선언되었기때문에
				// 84번의 블록을 벗어나면 소멸된다!
...
//				}
			...
			//인덱스: 손님이 선택한것 -1 ex) 1. 아메리카노 ---> 0인덱스에 있음
				...
				if(0<=cnt && cnt<=mCnt[num-1]) { //올바른입력이란 범위내에 입력 하는것
					break;
				}
			...
			res = money-res;
			if(res!=0) {
				System.out.println("거스름돈 "+res+"원 반환");
				System.out.println();
			}
